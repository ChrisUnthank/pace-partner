-- ============================================================================
-- FIX: Overall Economy Rating was blank on every interval session.
--
-- THE CAUSE
--
-- Overall Economy = 40% MEI + 25% Pace/HR Efficiency + 20% Fatigue + 15%
-- Stability, and returns null unless all four exist. MEI, Fatigue and
-- Stability all gained interval methodologies in 20260810020000. Pace/HR
-- Efficiency did not — its CTE filtered session_fatigue to
-- method = 'continuous_drift', which only exists for continuous sessions.
--
-- The scores were there all along. Counted for one athlete:
--
--     continuous_drift   84 sessions   (continuous)
--     thirds             19 sessions   (intervals)
--     halves             15 sessions   (intervals)
--     first_last          3 sessions   (intervals)
--
-- 37 interval sessions had an efficiency score that was being discarded by a
-- filter, which is why Z1 Recovery showed an Overall Economy of 65 while Z4
-- Threshold — 29 sessions, with MEI, Rhythm, Stability and Fatigue all
-- populated — showed nothing.
--
-- THE FIX: accept all four methods, preferring continuous_drift where it
-- exists, then thirds, halves, first_last in descending granularity.
--
-- Built from the LIVE definition, one CTE replaced.
--
-- AFTER RUNNING: Overall Economy appears on interval sessions. Note it is
-- then derived from a coarser drift method (thirds or halves rather than
-- continuous drift), so an interval session's rating is not exactly
-- comparable with a continuous one — the same caution that already applies
-- to Rhythm, Stability and Fatigue across the two structures.
-- ============================================================================
...
BEGIN
  ...
  -- FIXED: accept the interval drift methods, not just continuous_drift.
  --
  -- Pace/HR Efficiency was restricted to method = 'continuous_drift', which
  -- only ever exists for continuous sessions. Interval sessions DO have an
  -- efficiency score — session_fatigue computes them under 'thirds',
  -- 'halves' and 'first_last' — but this CTE filtered them out, so
  -- sf_pace_hr_efficiency was null for every interval session.
  --
  -- Overall Economy Rating requires all four of MEI, Pace/HR Efficiency,
  -- Fatigue and Stability, so that single filter blanked the headline score
  -- on every interval session. Z1 recovery runs showed 65; Z4 threshold
  -- showed nothing, despite having 29 sessions and the other three scores.
  --
  -- Counted on real data: 84 continuous_drift rows, plus 19 thirds, 15
  -- halves and 3 first_last — 37 interval sessions whose scores were being
  -- discarded.
  --
  -- Preference order rather than a bare filter: continuous_drift is the most
  -- precise method and wins where it exists, then thirds, halves, first_last
  -- in descending granularity. Recency breaks any remaining tie, as before.
  drift AS (
    ...
    WHERE sf.session_id IN (SELECT ar.id FROM all_runs ar)
      AND sf.method IN ('continuous_drift', 'thirds', 'halves', 'first_last')
      AND sf.efficiency_score IS NOT NULL
    ORDER BY
      sf.session_id,
      CASE sf.method
        WHEN 'continuous_drift' THEN 1
        WHEN 'thirds' THEN 2
        WHEN 'halves' THEN 3
        WHEN 'first_last' THEN 4
        ELSE 5
      END,
      sf.computed_at DESC
  ),
