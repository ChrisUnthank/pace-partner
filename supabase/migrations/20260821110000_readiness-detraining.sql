-- ============================================================================
-- READINESS — the formula change.
--
-- This CHANGES EVERY READINESS SCORE. Old and new figures are not comparable.
-- The backfill at the bottom is not optional: without it, history keeps the
-- old numbers and the two sit side by side in the same column.
--
-- Five faults from the review are fixed here. They are separate problems and
-- are commented separately below.
-- ============================================================================


-- ---------------------------------------------------------------------------
-- A place to say WHY a band is what it is.
--
-- A red band with no explanation is a number a coach cannot act on. The
-- injury cap below in particular needs to say so, or it looks like the
-- formula misfiring.
-- ---------------------------------------------------------------------------
ALTER TABLE public.athlete_load_daily
  ADD COLUMN IF NOT EXISTS readiness_reason text;

COMMENT ON COLUMN public.athlete_load_daily.readiness_reason IS
  'Short human-readable note on what drove the band, when something other than the plain score did. Null when the score speaks for itself.';
...
BEGIN
  ...
  ELSE
    ...
    eq_days_off := public.equivalent_days_off(_athlete_id, _date, 180);
  ...
  IF checkin_score_v IS NOT NULL AND answered < 3 AND confidence_v = 'high' THEN
    confidence_v := 'medium';
  END IF;
...
-- ============================================================================
-- THE BACKFILL IS SEPARATE — see BACKFILL_READINESS.sql.
--
-- Deliberately not run here. It rewrites every readiness row for every
-- athlete and takes real time, and a migration that silently rewrites years
-- of history is not something to discover afterwards. Run it knowingly.
-- ============================================================================
