...
function splitNumber(value: unknown): number | null {
...
function buildSplits(points: any[]): SplitRow[] {
  ...
  const ordered = [...points]
    .filter((p) => p && splitNumber(p.elapsed_s) != null)
    .sort((a, b) => (splitNumber(a.elapsed_s) ?? 0) - (splitNumber(b.elapsed_s) ?? 0));
  ...
  for (const point of ordered) {
    const type = normalizeSplitType(point.segment_type);
  ...
  return groups.map((grp, idx) => {
    ...
    const type = normalizeSplitType(first.segment_type);
    const durationS = Math.max(
      0,
      (splitNumber(last.elapsed_s) ?? 0) - (splitNumber(first.elapsed_s) ?? 0),
    );
    const distanceM = Math.max(
      0,
      (splitNumber(last.distance_m) ?? 0) - (splitNumber(first.distance_m) ?? 0),
    );
    const hrs = grp.map((p) => splitNumber(p.hr)).filter((v): v is number => v != null && v > 0);
    const paces = grp
      .map((p) => splitNumber(p.pace_sec_per_km))
      .filter((v): v is number => v != null && v > 0 && v <= 900);
    const cads = grp.map((p) => splitNumber(p.cadence)).filter((v): v is number => v != null && v > 0);
    let gain = 0;
    let loss = 0;
    let hasElevation = false;

    for (let i = 1; i < grp.length; i++) {
      const prev = splitNumber(grp[i - 1].elevation_m);
      const next = splitNumber(grp[i].elevation_m);
    ...
    const avgPace = distanceM > 0 && durationS > 0 ? (durationS / distanceM) * 1000 : avg(paces);
    ...
    return {
      ...
      avgHr: hrs.length ? Math.round(avg(hrs) ?? 0) : null,
      maxHr: hrs.length ? Math.max(...hrs) : null,
      avgCad: cads.length ? Math.round(avg(cads) ?? 0) : null,
      maxCad: cads.length ? Math.max(...cads) : null,
      elevGain: hasElevation ? Math.round(gain) : null,
      elevLoss: hasElevation ? Math.round(loss) : null,
    };
  });
}
      ...
      <CardContent>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.index} className={`${SPLIT_ROW_CLASS[r.type]} border-b last:border-b-0`}>
