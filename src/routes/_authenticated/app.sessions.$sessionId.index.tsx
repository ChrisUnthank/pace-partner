  ...
  const {
    ...
    queryFn: async ({ pageParam }) => {
      ...
      let q = supabase
        ...
        //
        // These two deliberately point opposite ways. Chaining descending
        // onto both put the evening session above the morning one inside the
        // same day, which reads backwards — a day is a unit you read
        // forwards, even in a list where the newest day is on top.
        //
        // The sort itself works because session_time_of_day is a Postgres
        // enum declared in chronological order (morning=1, afternoon=2,
        // evening=3), so ascending IS chronological rather than alphabetical.
        //
        // NULLS FIRST because planned sessions and manual entries (gym, races
        // not yet run) have no time_of_day — they belong at the top of the
        // day rather than buried under the completed ones. Postgres defaults
        // to NULLS LAST on ascending, so this has to be explicit.
