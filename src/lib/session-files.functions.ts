...
async function rebuildSessionFromAllFiles(sb: any, sessionId: string): Promise<void> {
  ...
  const isAutoGeneratedTitle = /^(Morning|Afternoon|Evening) (session|Run)$/.test(String(sess.title ?? ""));
  ...
  if (isAutoGeneratedTitle && anchorMs > 0) {
    ...
    // through) read as "Morning Run"; sessions classified as intervals
    // keep the more generic "session" label since "Run" implies one
    // unbroken effort. isIntervals is the same classification written to
    // structure below, so this always matches the session's final structure.
    recomputedTitle = isIntervals ? `${timeLabel} session` : `${timeLabel} Run`;
  }
        ...
        const { data: inserted, error: sessError } = await sb
          ...
          .insert({
            ...
            title: (() => {
              ...
              // this initial insert, so the title always reads as a Run here;
              // rebuildSessionFromAllFiles will relabel it "session" later if
              // the file turns out to contain interval structure.
