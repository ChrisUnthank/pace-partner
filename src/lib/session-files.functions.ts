...
async function rebuildSessionFromAllFiles(sb: any, sessionId: string): Promise<void> {
  ...
  if (anchorMs > 0) {
    ...
    // Boundary is < 12, not < 11. A session starting at 11:01 or 11:54 is a
    // late-morning run in anyone's language; the old cutoff labelled those
    // "Afternoon", which was wrong both as a title and — more importantly —
    // as the time_of_day value that drives same-day sort order everywhere.
    const timeLabel = localHour < 12 ? "Morning" : localHour < 16 ? "Afternoon" : "Evening";
    recomputedTimeOfDay = localHour < 12 ? "morning" : localHour < 16 ? "afternoon" : "evening";
