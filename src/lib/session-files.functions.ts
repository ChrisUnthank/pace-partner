...
function mapFitSport(sport?: string): string {
  ...
  if (s.includes("triathlon")) return "run"; // default triathlon to run
  return "run";
}
...
function parseGPX(xml: string) {
  ...
  return {
    ...
    sport:          null, // GPX has no standard sport field
  };
}
...
async function parseFIT(buffer: ArrayBuffer) {
  ...
  return await new Promise<{
    ...
    // Use Uint8Array instead of Buffer — Buffer is not available in edge runtime
    parser.parse(new Uint8Array(buffer), (err: any, data: any) => {
      ...
      const points = records.map((r: any) => ({
        elapsed_s:               r.elapsed_time ??
                                   (r.timestamp
                                     ? (new Date(r.timestamp).getTime() - t0) / 1000
                                     : 0),
        lat:                     r.position_lat,
        lng:                     r.position_long,
        ...
        hr:                      r.heart_rate,
        cadence:                 r.cadence,
        pace_sec_per_km:         r.speed && r.speed > 0.1
                                   ? 1000 / r.speed
                                   : null,
...
export const uploadAndParseSessionFile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Slice correctly to avoid passing extra bytes from a subarray view
    const arrayBuffer = buf.buffer.slice(
      buf.byteOffset,
      buf.byteOffset + buf.byteLength,
    );
 
    // Parse file first so we can use sport to set activity_type
    let parsed: {
      points:         any[];
      totalDistanceM: number;
      totalTimeS:     number;
    ...
    try {
      parsed =
        data.kind === "gpx"
          ? parseGPX(new TextDecoder().decode(buf))
          : await parseFIT(arrayBuffer);
    } catch (e: any) {
      parseError = String(e?.message ?? e);
      parsed = { points: [], totalDistanceM: 0, totalTimeS: 0, startedAt: null, sport: null };
    }
 
    // Determine activity type from FIT sport field, default to "run"
    const activityType = mapFitSport(parsed.sport ?? undefined);
 
    // Derive session date from file start time or today
    ...
    // Insert session_files record (with parse error if applicable)
    ...
    // If parse failed, return early with error
    if (parseError) {
      return { file: fileRow, points: 0, error: parseError };
    }
 
    // Insert trace points into raw_session_points in chunks of 500
    if (parsed.points.length) {
      ...
      // Recompute session-level work aggregates from parsed points
      const hrs   = parsed.points
        .map((p) => p.hr)
        .filter((x): x is number => typeof x === "number");
      const paces = parsed.points
        .map((p) => p.pace_sec_per_km)
        .filter((x): x is number => typeof x === "number" && x > 0);
      const cads  = parsed.points
        .map((p) => p.cadence)
        .filter((x): x is number => typeof x === "number" && x > 0);
...
export const submitCheckout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Save post-session insights for each session
    ...
    // Save end of day note independently on the athlete's daily checkin
    // rather than attaching it to a specific session insight row
    ...
    // Update last checkout timestamp on athlete record
    await sb
      .from("athletes")
...
export const sendReminder = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (d: { athleteId: string; kind: string; message?: string }) => d,
  )
  .handler(async ({ data, context }) => {
