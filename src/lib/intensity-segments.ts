...
//
// Zone scheme matches this app's actual live recompute_session_zones
// function exactly (confirmed via pg_get_functiondef against the real
// database, not assumed from an earlier, superseded migration file that
// was still sitting in the repo) — six zones, z1-z6, driven by five real
// boundaries (hr_z1_max..hr_z5_max / pace_z1_max_sec_per_km..
// pace_z5_max_sec_per_km), z6 as the open-ended catch-all above z5. This
// mirrors the app's own ZONE_LABEL convention (z1 Recovery, z2 Easy/
// Aerobic, z3 Steady/Tempo, z4 Threshold, z5 VO2, z6 Anaerobic/Max).
//
// WHY THIS MODULE EXISTS
// A plain HR-threshold lookup (what recompute_session_zones does in the
// DB, and what this module's caller falls back to when no raw point
// trace exists) cannot tell these three things apart, because they can
// produce an identical average HR over a long window:
//   1. A continuous race that drifted into Z5/Z6 purely from cardiac
//      drift (heat, dehydration, duration) — the energy system used the
//      whole way stayed aerobic; the elevated HR is a lagging,
//      plateauing artifact, not evidence of anaerobic contribution.
//   2. A genuine mid-race surge (a hard final kilometre, surging to pass
//      someone) — a real, shorter-duration harder effort embedded inside
//      an otherwise-aerobic continuous block.
//   3. A genuine short finishing sprint (last 100-400m) — real anaerobic
//      contribution, but brief.
//
// HR alone can't distinguish these because it's a LAGGING indicator
// (takes 30-90s to catch up to a real effort change) and PLATEAUS at high
// intensity (a steady hard race and a sprint can show the same flat-line
// high HR). This module cross-checks HR against two other signals raw
// GPS data actually provides:
...
//     capped in duration; a long unbroken high-HR block that ISN'T
//     embedded surges is drift, not effort.
...
//     actually increase, only the cardiovascular cost of sustaining it
//     did.
//
...
export type HrZoneKey = "z1" | "z2" | "z3" | "z4" | "z5" | "z6";

const ZONE_KEYS: HrZoneKey[] = ["z1", "z2", "z3", "z4", "z5", "z6"];

// The two zones a genuine short/embedded effort can land in — z5 (VO2)
// and z6 (Anaerobic/Max). The continuous-effort cap folds these down to
// z4 (Threshold) for anything NOT confirmed as a genuine effort.
const HIGH_ZONES: HrZoneKey[] = ["z5", "z6"];
...
export function hrZoneFor(hr: number, zones: HrZoneBoundaries): HrZoneKey {
  if (zones.z1Max != null && hr <= zones.z1Max) return "z1";
  if (zones.z2Max != null && hr <= zones.z2Max) return "z2";
  if (zones.z3Max != null && hr <= zones.z3Max) return "z3";
  if (zones.z4Max != null && hr <= zones.z4Max) return "z4";
  if (zones.z5Max != null && hr <= zones.z5Max) return "z5";
  return "z6";
}

function emptyZoneMap(): Record<HrZoneKey, number> {
  return { z1: 0, z2: 0, z3: 0, z4: 0, z5: 0, z6: 0 };
}

// Raw Time-at-Intensity per zone (seconds), plain per-point-threshold sum,
// no drift correction — what a short continuous effort (under the cap)
// uses as-is, and what the refined calculation starts from.
export function computeTaiByZone(points: IntensityPoint[], zones: HrZoneBoundaries): Record<HrZoneKey, number> {
  const totals = emptyZoneMap();
...
// Same idea as computeTaiByZone but for distance (meters) instead of
// time — session_zone_time tracks both dimensions, so a corrected
// seconds total needs a correspondingly corrected meters total or the
// two go inconsistent with each other for the affected zone.
export function computeMetersByZone(points: IntensityPoint[], zones: HrZoneBoundaries): Record<HrZoneKey, number> {
  const totals = emptyZoneMap();
  for (let i = 1; i < points.length; i++) {
    const prev = points[i - 1];
    const cur = points[i];
    if (cur.hr == null || prev.distanceM == null || cur.distanceM == null) continue;
    const dm = cur.distanceM - prev.distanceM;
    if (dm <= 0) continue;
    totals[hrZoneFor(cur.hr, zones)] += dm;
  }
  return totals;
}
...
// Contiguous stretches where HR sits at/above z5 (VO2) — z5 or z6. Runs
// shorter than minDurationS are dropped as sensor noise (a single
// spurious high HR sample isn't a real effort). Not exported as the
// "answer" on its own — each run still needs onset-slope and pace-surge
// evaluation before it's trusted as genuine (see classifyRun).
export function findHighIntensityRuns(points: IntensityPoint[], zones: HrZoneBoundaries, minDurationS = 15): IntensityRun[] {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    const isHigh = hr != null && HIGH_ZONES.includes(hrZoneFor(hr, zones));
...
// settling into pace over the first 1-2km shows a gentle one. Returns
// null when there's no usable baseline point (e.g. the run starts at the
...
// The block's own baseline pace, computed excluding whatever high-
// intensity runs were found — this is "what pace was this effort
// actually holding outside the flagged stretches," the reference point a
// run gets compared against to see if it's really a surge or just
// elevated HR at the same pace.
...
// EITHER a steep onset slope OR a real pace surge — elevated HR by
// itself is deliberately insufficient evidence, since that's exactly the
// signal cardiac drift also produces. Among confirmed runs, duration
// decides anaerobic (short, explosive — a finishing kick) vs vo2
// (sustained harder effort, e.g. a hard final kilometre).
...
// A run found by findHighIntensityRuns is one CONTIGUOUS stretch of
// z5/z6 points — but a real race commonly drifts gradually into a high
// HR zone and THEN kicks, with no dip back down to a lower zone in
// between the drift and the kick. That whole stretch shows up as a
// single run. Classifying it as one atomic unit would wrongly tar the
// genuine kick with the drift's "not confirmed" verdict (or, just as
// wrong, let the kick's real pace surge drag the drifted portion's
// average enough to look like a false positive for the whole thing).
// This scans for the single best split point within a run's back half
// where the tail (candidate kick) is genuinely faster than the head
// (candidate drift) by at least minSurgePct — if found, the run gets
// evaluated as two pieces instead of one. Returns null when no such
// internal acceleration exists, meaning the run really is uniform and a
// single evaluation applies as-is.
...
// Splits a run into [head, kick] sub-runs if findKickSplitWithinRun finds
// a genuine internal acceleration, otherwise returns the run unchanged as
// a single-element array — callers always get back one or more runs to
...
export interface ZoneTimeResult {
  ...
  meters: Record<HrZoneKey, number>;
  detectedRuns: DetectedRun[];
}

// Main entry point for a continuous block (steps.reps <= 1, not a
// recovery step). Blocks at or under continuousCapMinutes are returned as
// plain TAI with no correction — a continuous effort short enough to
// genuinely BE anaerobic/VO2-dominant (a hard 1500-3000m time trial run
// as one block) shouldn't have that flattened out just for being logged
// as a single rep. Above that length, every z5/z6 stretch gets evaluated:
// confirmed genuine efforts (anaerobic or vo2) keep their real zone time,
// everything else — the drift — gets folded down into z4 (Threshold)
// rather than inflating z5/z6 with sustained cardiac drift.
export function computeRefinedContinuousZoneTime(
  points: IntensityPoint[],
  zones: HrZoneBoundaries,
  continuousCapMinutes = 12,
): ZoneTimeResult {
  if (points.length < 2) return { seconds: emptyZoneMap(), meters: emptyZoneMap(), detectedRuns: [] };

  const totalDurationS = points[points.length - 1].elapsedS - points[0].elapsedS;

  if (totalDurationS <= continuousCapMinutes * 60) {
    return { seconds: computeTaiByZone(points, zones), meters: computeMetersByZone(points, zones), detectedRuns: [] };
  }
  ...
  const seconds = emptyZoneMap();
  const meters = emptyZoneMap();
  ...
  for (let i = 1; i < points.length; i++) {
    ...
    if (HIGH_ZONES.includes(zone) && !genuineIndices.has(i)) {
      zone = "z4";
    }
    seconds[zone] += dt;

    if (prev.distanceM != null && cur.distanceM != null) {
      const dm = cur.distanceM - prev.distanceM;
      if (dm > 0) meters[zone] += dm;
    }
  }

  return { seconds, meters, detectedRuns };
}
...
// — HR never fully drops between reps) per the classic work:rest
// convention. Each rep's own average HR already classifies it reasonably
// against threshold on its own (the bug this module exists for is
// specific to continuous single blocks) — this is exposed as an
// additional descriptive signal for anything that wants to label a
// session's overall interval character, not a correction to per-rep zone
// time. NOT WIRED INTO ANYTHING YET as of this module's introduction —
// exists and is tested, but no caller uses it.
export interface WorkRestSparsity {
  avgWorkDurationS: number;
  avgRestDurationS: number | null;
  workToRestRatio: number | null;
  recoveryDepthBpm: number | null;
