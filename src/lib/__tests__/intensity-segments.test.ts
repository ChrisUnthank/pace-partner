...
// Matches a real athlete's actual zone card: z1<=124, z2<=143, z3<=162,
// z4<=172, z5<=186, z6 open (Recovery/Easy-Aerobic/Steady-Tempo/
// Threshold/VO2/Anaerobic-Max) — confirmed against the live
// recompute_session_zones function, not assumed.
const ZONES: HrZoneBoundaries = { z1Max: 124, z2Max: 143, z3Max: 162, z4Max: 172, z5Max: 186 };
...
function buildPoints(
  segments: { durationS: number; hr: number; hrEnd?: number; paceSecPerKm: number }[],
): IntensityPoint[] {
  ...
  for (const seg of segments) {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      points.push({ elapsedS: t, hr, paceSecPerKm: seg.paceSecPerKm, distanceM: cumulativeDistance });
      t += 5;
      cumulativeDistance += (5 / seg.paceSecPerKm) * 1000;
      lastHr = hr;
  ...
  points.push({ elapsedS: t, hr: lastHr, paceSecPerKm: lastPace, distanceM: cumulativeDistance });
  return points;
}

describe("hrZoneFor", () => {
  it("buckets ascending HR into the right zone", () => {
    expect(hrZoneFor(115, ZONES)).toBe("z1");
    expect(hrZoneFor(135, ZONES)).toBe("z2");
    expect(hrZoneFor(155, ZONES)).toBe("z3");
    expect(hrZoneFor(168, ZONES)).toBe("z4");
    expect(hrZoneFor(180, ZONES)).toBe("z5");
    expect(hrZoneFor(195, ZONES)).toBe("z6");
  });
});

describe("computeTaiByZone", () => {
  it("sums seconds per zone from a plain point series", () => {
    const points = buildPoints([{ durationS: 60, hr: 115, paceSecPerKm: 330 }]);
    const tai = computeTaiByZone(points, ZONES);
    expect(tai.z1).toBe(60);
    expect(tai.z4).toBe(0);
  });
});
...
describe("Pattern 1 — Continuous race, cardiac drift only (no real kick)", () => {
  ...
  // steady/threshold pace, then GRADUALLY drifts up into z5 (VO2) HR over
  ...
  const points = buildPoints([
    { durationS: 120, hr: 118, hrEnd: 138, paceSecPerKm: 260 }, // settle-in
    { durationS: 900, hr: 138, hrEnd: 160, paceSecPerKm: 240 }, // steady race effort
    { durationS: 480, hr: 160, hrEnd: 178, paceSecPerKm: 241 }, // gradual drift into z5, pace UNCHANGED
  ]);
  ...
  it("folds the drifted time into z4 (threshold) instead of z5", () => {
    const result = computeRefinedContinuousZoneTime(points, ZONES, 12);
    expect(result.seconds.z5).toBe(0);
    expect(result.seconds.z4).toBeGreaterThan(0);
  });

  it("the naive (unrefined) TAI would have wrongly shown real z5 time", () => {
    const naive = computeTaiByZone(points, ZONES);
    expect(naive.z5).toBeGreaterThan(0); // confirms the bug this module fixes is real
  });
});
...
describe("Pattern 2 — Continuous race WITH a genuine final sprint", () => {
  ...
  const points = buildPoints([
    { durationS: 120, hr: 118, hrEnd: 138, paceSecPerKm: 260 },
    { durationS: 900, hr: 138, hrEnd: 160, paceSecPerKm: 240 },
    { durationS: 480, hr: 160, hrEnd: 178, paceSecPerKm: 241 }, // drift, same as pattern 1
    { durationS: 45, hr: 192, paceSecPerKm: 195 }, // genuine sprint: sharp HR + real pace surge, into z6
  ]);
  ...
  it("the genuine kick's time still counts toward z6", () => {
    const result = computeRefinedContinuousZoneTime(points, ZONES, 12);
    expect(result.seconds.z6).toBeGreaterThan(0);
  });
});
...
describe("Pattern 3 — Short continuous effort under the cap (no correction needed)", () => {
  ...
  const points = buildPoints([{ durationS: 270, hr: 190, paceSecPerKm: 200 }]);
...
describe("isPaceSurge", () => {
  it("confirms a real surge when pace is meaningfully faster than baseline", () => {
    const run = { startIdx: 0, endIdx: 1, startS: 0, endS: 10, durationS: 10, avgHr: 190, avgPaceSecPerKm: 195 };
    expect(isPaceSurge(run, 240)).toBe(true);
  });

  it("rejects a surge when pace barely changed (the drift signature)", () => {
    const run = { startIdx: 0, endIdx: 1, startS: 0, endS: 10, durationS: 10, avgHr: 178, avgPaceSecPerKm: 241 };
    expect(isPaceSurge(run, 240)).toBe(false);
  });
});

describe("classifyRun", () => {
  const run = { startIdx: 0, endIdx: 1, startS: 0, endS: 45, durationS: 45, avgHr: 192, avgPaceSecPerKm: 195 };
...
describe("Work:rest sparsity distinguishes anaerobic reps from VO2 intervals", () => {
...
describe("findHighIntensityRuns", () => {
  it("drops sub-noise-threshold blips", () => {
    const points = buildPoints([
      { durationS: 60, hr: 138, paceSecPerKm: 260 },
      { durationS: 5, hr: 190, paceSecPerKm: 200 }, // 5s blip — should be dropped
      { durationS: 60, hr: 138, paceSecPerKm: 260 },
    ]);
  ...
  it("keeps a run at or above the minimum duration", () => {
    const points = buildPoints([
      { durationS: 60, hr: 138, paceSecPerKm: 260 },
      { durationS: 30, hr: 190, paceSecPerKm: 200 },
      { durationS: 60, hr: 138, paceSecPerKm: 260 },
    ]);
    const runs = findHighIntensityRuns(points, ZONES, 15);
...
describe("computeBaselinePace", () => {
  it("excludes flagged runs from the baseline average", () => {
    const points = buildPoints([
      { durationS: 60, hr: 145, paceSecPerKm: 250 },
      { durationS: 30, hr: 190, paceSecPerKm: 180 },
    ]);
    const runs = findHighIntensityRuns(points, ZONES, 15);
